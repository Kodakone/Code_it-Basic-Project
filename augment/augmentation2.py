...
def save_yolo_label(label_path: Path, boxes_xyxy, cls_ids, img_w, img_h):
    ...
    lines = []
    for (x1, y1, x2, y2), c_id in zip(
        boxes_xyxy, cls_ids
    ):  # bbox와 category id 하나씩 처리
        # c_id : 0,1,2...
...
if "check_each_transform" in sig:
    bp_kwargs["check_each_transform"] = (
        False  # 매 변환마다 엄격 체크를 줄임(대신 아래에서 직접 정리)
    )
...
top_class_set = {
    cid for cid, cnt in class_count.items() if cnt >= top_min
}  # 상위클래스
mid_class_set = {
    cid for cid, cnt in class_count.items() if mid_min <= cnt <= mid_max
}  # 중위클래스
...
underfilled_mid_set = get_underfilled_mid_set(
    cur_bbox_count, target_bbox_count, mid_class_set
)
...
print(
    "num src imgs:",
    len(list(IMG_DIR0.glob("*.png"))) + len(list(IMG_DIR0.glob("*.jpg"))),
)
...
for fname in tqdm(sorted(train_fnames), desc="TRAIN preprocess"):
    ...
    for k in range(aug_n):
        try:
            transformed = aug_tf(
                image=img_bgr, bboxes=clean_bboxes, class_labels=clean_cls_ids
            )
        except ValueError:
        ...
        if is_safe_src and (
            len(final_catids_set & rare_safe_set) == 0
        ):  # safe없으면 적용x
            skipped_no_safe += 1
            continue
        ...
        if fname in cond_mid_img_set:
            ...
            if (final_catids_set & head_catids) or (
                final_catids_set & always_with_3351
            ):
...
underfilled_mid_set_after = get_underfilled_mid_set(
    cur_bbox_count, target_bbox_count, mid_class_set
)
